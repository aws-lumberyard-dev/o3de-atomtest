...
_ASSET_PATH = Path(cmds.file(q=True, l=True)[0]).parent.resolve()
print(_ASSET_PATH)
...
class StingrayPBS(object):
    def __init__(self):
        ...
        # TO DO: Add some error checking, make sure something is selected
...
for i in range(len(strpbs.mat)):
    ...
    print('writting: {0}'.format(Path(_ASSET_PATH, "{}.material".format(str(strpbs.mat[i]))).resolve()))
